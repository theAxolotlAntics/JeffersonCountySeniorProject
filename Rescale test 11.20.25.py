...
class App(tk.Tk):
    ...
    def CreateToolbar(self): #creates a tool bar with navigation buttons
        ...
        ttk.Entry(bar, textvariable=self.SearchInput, width=30).pack(side="right", padx=4)
    ...
    def ApplyFilters(self):
        ...
        if query:
            mask = data["Address"].astype(str).str.lower().str.contains(query)
            for col in ["ParcelID", "Owner", "Created", "Modified", "StreetNum", "City", "First", "Last"]:
                if col in data.columns:
                    #if the contents of the search bar are contained at all in any of the values of the columns
                    mask |= data[col].astype(str).str.lower().str.contains(query)
            data = data[mask]
...
#--------------------------------------------------------------------------------------------------------
        ...
        # Grid weights so the popup lays out correctly when resized:
        # Row 0 -> image area (should expand vertically a bit)
        # Row 1 -> info area (should expand)
        # Row 2 -> notes (should expand)
        # Row 3 -> buttons (fixed)
        ...
        # Column 0 -> main content (image + info + notes)
        # Column 1 -> right column / sidebar
...
#----------------------------------------------------------------------------------------
        ...
        # Image Frame (top-left). Use grid everywhere and allow it to expand.
        ImageFrame = ttk.Frame(win, relief="solid", padding=0)
        ImageFrame.grid(row=0, column=0, columnspan=2, sticky="nsew", padx=5, pady=5)
        # make its internal cell expand
        ImageFrame.grid_rowconfigure(0, weight=1)
        ImageFrame.grid_columnconfigure(0, weight=1)

        # Label that will hold the image use grid and sticky so it expands with the frame
...
#------------------------------------------------------------------------------------------
    ...
    # If there's no image, show the placeholder text and exit
            if not OriginalImage:
                ImageLabel.config(image="", text=ImageLabel.cget("text"))
                return

    # Only respond to ImageFrame configure events to avoid duplicate calls.
    # If event.widget is present and is not ImageFrame, still compute based on ImageFrame geometry.
            try:
                FrameW = max(1, ImageFrame.winfo_width())
                FrameH = max(1, ImageFrame.winfo_height())
            except Exception:
                FrameW = ImageFrame.winfo_width()
                FrameH = ImageFrame.winfo_height()

        # If the frame isn't visible yet, try again shortly
    ...
    # get image size
        ...
        # decide target size preserving aspect ratio
            image_ratio = iw / ih
            frame_ratio = FrameW / FrameH if FrameH != 0 else 1

            if frame_ratio > image_ratio:
            # frame is wider relative to height -> use frame height
                TargetH = FrameH
                TargetW = int(TargetH * image_ratio)
            else:
                TargetW = FrameW
                TargetH = int(TargetW / image_ratio)

        # do not upscale beyond original image size (set to True if you want upscaling)
            allow_upscale = False
        ...
        # skip if unchanged
            if getattr(ImageLabel, "_last_size", None) == (TargetW, TargetH):
                return
            ImageLabel._last_size = (TargetW, TargetH)

        # create a resized PhotoImage and set it
            try:
            ...
            # fallback: show placeholder text
                ImageLabel.config(image="", text=f"Image error: {e}")

#-----------------------------------------------------------------------------------------------

  # update image dynamically only when ImageFrame changes
            ImageFrame.bind("<Configure>", ResizeImage)
# do an initial resize after the window has a chance to layout
