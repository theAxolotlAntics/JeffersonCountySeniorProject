...
class App(tk.Tk):
    ...
    def Selected(self, event):
        ...
        EditBtn.grid(row=2, column=0, sticky="ew", padx=8, pady=(6, 8))
        NoteBtn.grid(row=2, column=1, sticky="ew", padx=8, pady=(6, 8))
    ...
    def EditProperty(self, idx, parent_win=None):
        ...
        save_btn = ttk.Button(btnfrm, text="Save", command=on_save)
        cancel_btn = ttk.Button(btnfrm, text="Cancel", command=on_cancel)
    ...
    def AddNote(self, idx, parent_win=None):
        ...
        def on_add():
            ...
            if "Notes" not in self.df.columns:
                self.df["Notes"] = ""
            current = str(self.df.at[idx, "Notes"]) if not pd.isna(self.df.at[idx, "Notes"]) else ""
        ...
        ttk.Button(btnframe, text="Add", command=on_add).pack(side="right", padx=6)
        ttk.Button(btnframe, text="Cancel", command=on_cancel).pack(side="right", padx=6)
